...
const i18n = {
  en: {
    ...
    'alipay.tag1': 'UX research',
    'alipay.tag2': 'problem reframing',
    'alipay.tag3': 'financial AI boundary',
    'alipay.hover': 'Research turns a vague business goal into a decision mechanism users can actually perceive.',
    ...
    'brokerage.hover': 'The object points to the system underneath: journey, state, risk, and recovery rules.',
    ...
    'agent.hover': 'AI value is shown as editable, system-consistent output rather than visual novelty.',
    ...
    'footer.copy': '© 2026 Lynn. A quiet lab for product judgment, systems, and craft.',
  ...
  zh: {
    ...
    'brokerage.hover': '物件指向背后的系统：链路、状态、风险和恢复规则。',
    'agent.kicker': 'AI 工具',
    ...
    'agent.tag2': '设计系统',
    'agent.tag3': '评估标准',
    'agent.hover': 'AI 价值被表达为可编辑、符合设计系统的产物，而不是视觉新奇感。',
    'lab.kicker': '实验 / 研究',
    'lab.title': '实验场',
    ...
    'lab.archive': '早期作品归档',
    'footer.copy': '© 2026 Lynn. 一个关于产品判断、系统与工艺的安静实验室。',
    'footer.resume': '简历',
    'footer.about': '关于',
    'footer.contact': '联系',
...
function setupInkField() {
  ...
  let height = 0;
  let dpr = 1;
  let pointer = { x: -1000, y: -1000, px: -1000, py: -1000, active: false };
  const pulses = [];
  const points = [];
  ...
  function buildPoints() {
    points.length = 0;
    const count = Math.max(42, Math.min(110, Math.floor((width * height) / 17000)));
  ...
  function color(alpha) {
    return root.dataset.theme === 'dark'
      ? `rgba(243,237,226,${alpha})`
      : `rgba(37,37,37,${alpha})`;
  }

  function accent(alpha) {
    return root.dataset.theme === 'dark'
      ? `rgba(224,198,109,${alpha})`
      : `rgba(111,111,104,${alpha})`;
  }

  function addPulse(x, y, strength = 1) {
    pulses.push({ x, y, r: 8, life: 1, strength });
    if (pulses.length > 16) pulses.shift();
  }

  function drawPulses() {
    pulses.forEach((pulse) => {
      pulse.life *= .952;
      pulse.r += 2.6 * pulse.strength;
      const gradient = ctx.createRadialGradient(pulse.x, pulse.y, 0, pulse.x, pulse.y, pulse.r * 7);
      gradient.addColorStop(0, accent(pulse.life * .09));
      gradient.addColorStop(.38, color(pulse.life * .018));
      gradient.addColorStop(1, 'rgba(0,0,0,0)');
      ctx.fillStyle = gradient;
      ctx.beginPath();
      ctx.arc(pulse.x, pulse.y, pulse.r * 7, 0, Math.PI * 2);
      ctx.fill();
    });
    for (let i = pulses.length - 1; i >= 0; i -= 1) {
      if (pulses[i].life < .04) pulses.splice(i, 1);
    }
  }
  ...
  function drawField(time) {
    points.forEach((point) => {
      ...
      point.vx += (dx / dist) * force * 1.8;
      point.vy += (dy / dist) * force * 1.8;
      point.vx += (point.ox - point.x) * .006;
      point.vy += (point.oy - point.y) * .006;
      point.vx *= .9;
      point.vy *= .9;
      point.x += point.vx + Math.sin(time * .001 + point.phase) * .08;
      point.y += point.vy + Math.cos(time * .001 + point.phase) * .08;
    });
  ...
  document.addEventListener('pointerdown', (event) => {
    addPulse(event.clientX, event.clientY, 1.25);
  }, { passive: true });

  resize();
  addPulse(width * .72, height * .28, .72);
